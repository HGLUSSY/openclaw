...
function printUsage(): void {
  process.stdout.write(
    "Usage: node --import tsx scripts/android-sync-versioning.ts --check [--require-mobile-release] [--root dir]\n",
  );
}

function main(argv = process.argv.slice(2)): number {
  const requireMobileRelease = argv.includes("--require-mobile-release");
  const options = parseVersionSyncArgs(argv.filter((arg) => arg !== "--require-mobile-release"));
  if (options.help) {
    printUsage();
    return 0;
  }
  ...
  checkAndroidVersioning({ requireMobileRelease, rootDir: options.rootDir });
