...
| Runner                          | Jobs                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ubuntu-24.04`                  | `security-fast`, `openclaw/ci-gate`, hybrid `preflight`, manual CI dispatch and non-canonical repository fallbacks, CodeQL security and quality scans, workflow-sanity, labeler, auto-response, the standalone Docs workflow, the whole Install Smoke workflow, all configurable CI jobs in `github` mode, and the remaining light lanes plus rerun Blacksmith lanes in `hybrid` mode. The GitHub/hybrid planner profile expands the Node matrix, QA Smoke to six parts, core oxlint across five stripes, and the core test-type graphs across five stripes while leaving extension/scripts lint plus optional UI and format checks in the existing `check-lint` row and the extensions/root/scripts type tail in the `check-test-types` row. |
| `blacksmith-4vcpu-ubuntu-2404`  | `preflight` in all-Blacksmith mode, `pnpm-store-warmup`, `native-i18n`, `checks-fast-core` except QA Smoke CI, plugin/channel contract shards, most bundled/lower-weight Linux Node shards, `check-*` lanes except `check-lint`, selected `check-additional-*` shards, `check-docs`, and `skills-python`                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| `blacksmith-12vcpu-macos-26`    | `macos-swift` on `openclaw/openclaw`; untrusted authors fall back to `macos-26`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
Short whole-config or native-sharded plugin fallback envelopes from different configs can share a job in pairs, using the compact packer's existing 94-second expanded large-runner admission budget. Each envelope retains its original environment and native shard arguments. Process-bounded configs keep their existing jobs: their averaged cost estimates can hide expensive imports in an individual chunk. Partitions of the same config and jobs with runtime prerequisites also remain separate. Runner placement, worker limits, the default timeout and the matrix concurrency cap stay unchanged; a failure retains the serial runner's normal stop-on-failure behavior. This reduces repeated checkout and setup without changing the conservative fallback-row bound above.
...
Expanded large/small jobs admit 94/114 predicted seconds and retain the 96-row hybrid and 112-row GitHub compact caps. Plugin fallback contributes at most 78 rows; eligible pairs reduce the emitted count. Exclusive-group admission remains 150 seconds. The PR-only performance lifecycle file uses a 136-second fallback, rounded up from the larger native shard span of 127.288/135.808 seconds in runs 33532741896/33545657559. Canonical main pushes omit that tooling family. Trusted contributor forks select the GitHub planner profile but can still run those rows on Blacksmith, so their unchanged 112-row cap owns the conservative PR bound. The workflow has 88 other potential job rows across backends: 13 nonmatrix rows and 75 matrix rows, including both Swift and both iOS build phases. Exclude the four unconditionally hosted iOS build/capture rows and conservatively count the remaining 84 as Blacksmith admissions, including other hosted and skipped rows. This gives a broad-PR bound of `112 + 78 + 84 = 274`; canonical main uses the 96-row hybrid cap without PR fallback descriptors, giving a bound of 180. The command splits fit inside those caps, so their rows are not added again. Two active main slots plus both pending successors and the observed peak of nineteen PR arrivals, including superseded heads, give `4 × 180 + 19 × 274 = 5,926` registrations in five minutes. The live bucket rechecked on 2026-09-01 was 10,000; this leaves 74 below the 6,000 operating target before additional release and adjacent-repository arrivals. Npm qualification adds three registrations per arrival compared with its former serial layout; budget all six current npm jobs per qualification inside five minutes. Further fanout needs reductions elsewhere or a narrower, evidenced arrival bound. This is an arrival-envelope estimate, not an organization-wide admission cap: re-evaluate emitted rows, adjacent repositories and queued-run overlap before increasing fanout. An unused registration bucket does not prove physical runner capacity.
