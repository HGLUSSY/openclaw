...
import { createLazyCoreHandlers, lazyHandlerModule } from "./lazy-core-handlers.js";
...
const handlerFamilyOrder = Object.keys(CORE_GATEWAY_HANDLER_MODULES);
// The descriptor table supplies typed families; retain the existing loader insertion order.
const coreGatewayHandlerFamilies = [...listCoreGatewayHandlerMethodNames()].toSorted(
  ([left], [right]) => handlerFamilyOrder.indexOf(left) - handlerFamilyOrder.indexOf(right),
);

export const coreGatewayHandlers: GatewayRequestHandlers = Object.fromEntries(
  coreGatewayHandlerFamilies.flatMap(([family, methods]) =>
    Object.entries(
      createLazyCoreHandlers({
        methods,
        loadHandlers: lazyHandlerModule(
          CORE_GATEWAY_HANDLER_MODULES[family],
          (handlers) => handlers,
        ),
      }),
    ),
